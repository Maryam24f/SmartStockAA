...
function Maintenance() {
  const [Rlist, setRlist] = useState([
    ...
    {
      id: 6,
      branch: "F9", // Branch Name
      ACategory: "consumbale", // Asset Category
      Aname: "Garbage bag", // Asset Type
      au: "rim",
      month: "2024-01-23", // Date
      quantity: "5",
  ...
  const handleDropdownR = (event, month, branch) => {
    ...
    console.log("updated"+statusByMonthBranch)
  };
  ...
  const handleFilterByApproveProcess = (value) => {
    ...
    if (activeTab === "requests") {
      // Filter data list
      const filteredData = data.filter((row) => row.selectedValue === value);
  ...
  <table className="min-w-full divide-y divide-gray-200 border-2 border-black shadow-md bg-white rounded-md">
    ...
    <tbody>
      ...
      {[...uniqueMonths].map((month) => {
        ...
        const branchesForMonth = Rlist.filter((item) => item.month === month)
        ...
        return branchesForMonth.map((branch, index) => {
          ...
          return (
            <tr key={`${month}-${branch}`} className="border-black">
              ...
              {/* Render branch */}
              <td className="border border-black" onClick={()=> handleShowDetails(month,branch)}>{branch}</td>
              {/* Render status */}
              <td className="border border-black">
                <select
                  className=""
                  value={status}
                  onChange={(e) => handleDropdownR(e, month, branch)}
                >
                  {options2.map((option) => (
                    <option key={option} value={option}>
                      {option}
